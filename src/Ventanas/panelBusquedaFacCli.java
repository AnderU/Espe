...
public class panelBusquedaFacCli extends JPanel {
	...
	private TablaFacturasClientesResumen modeloFactProvRe;
	public TablaFacturasClientesResumen getModeloFactProvRe() {
	...
	public void cargaTabla(String criterio)
	{
		...
		for (int i=0; i<row; i++)
 		...
 		ResultSet rs=ConectorBD.bdMySQL.Select("facturasclientes", "*", criterio);
 		try {
 			while (rs.next())
 			{
 				FacturasClientesC a=new FacturasClientesC();
 				
 				a.setIdFactCli(rs.getObject(1).toString());
 				a.setnFactura(rs.getObject(2).toString());
 				a.setIdFormaCobro(rs.getObject(3).toString());
 				a.setIdCliente(rs.getObject(4).toString());
 				a.setFecha(rs.getObject(5).toString());
 				if (rs.getObject(6)!=null)
 					a.setFechaCobro(rs.getObject(6).toString());
 				a.setIva(rs.getObject(7).toString());
 				a.setObservaciones(rs.getObject(8).toString());
 				ResultSet rs1=ConectorBD.bdMySQL.SelectAux("clientes", "Cliente", "Id="+a.getIdCliente());
 				rs1.next();
 				a.setCliente(rs1.getObject(1).toString());
 				rs1=ConectorBD.bdMySQL.SelectAux("detallefacturasclientes", "SUM(Precio*Cantidad)", "IdFactura="+a.getIdFactCli());
 				rs1.next();
	...
	public panelBusquedaFacCli() {
		...
		modeloFactProvRe= new TablaFacturasClientesResumen(vectorTabla,columnNames);
